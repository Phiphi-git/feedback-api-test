...
Write-Host "`n⚙️  Entraînement du modèle en cours..." -ForegroundColor Cyan
...
if (Test-Path "sentiment_model.pkl") {
    ...
    Write-Host "`n📊 Modèle prêt pour le déploiement!" -ForegroundColor Green
} else {
    Write-Host "❌ Le fichier sentiment_model.pkl n'a pas été créé" -ForegroundColor Red
    exit 1
}

Write-Host "`n✨ Entraînement terminé avec succès!`n" -ForegroundColor Green
